...
const API = 'http://localhost:8080/api/v1/inventory/ordenes';
          ...
          <table hlmTable class="w-full">
            <thead hlmTHead [ngClass]="headerColor">
              <tr hlmTr>
                ...
                <th hlmTh class="cursor-pointer select-none" (click)="toggleSort('tipoEventoNombre', $event)">
                  <div class="flex items-center gap-1">Tipo
                    @if (sortDir('tipoEventoNombre') === 'asc')   { <ng-icon hlmIcon size="sm" name="lucideChevronUp" /> }
                    @else if (sortDir('tipoEventoNombre') === 'desc') { <ng-icon hlmIcon size="sm" name="lucideChevronDown" /> }
                  </div>
                </th>
            ...
            <tbody hlmTBody>
              @for (o of pageItems(); track o.id; let odd = $odd) {
                ...
                >
                  ...
                  <td hlmTd class="text-muted-foreground">{{ o.tipoEventoNombre ?? '—' }}</td>
      ...
      <ng-template brnDialogContent>
        <hlm-dialog-content class="sm:max-w-md" [showCloseButton]="false">
          <div class="bg-destructive text-destructive-foreground flex items-center gap-2 px-4 h-11 -mx-6 -mt-6 mb-2 rounded-t-lg">
            <ng-icon hlmIcon size="sm" name="lucideClipboardList" />
            <h2 class="text-sm font-semibold">¿Eliminar orden?</h2>
          </div>
          <p class="text-sm text-muted-foreground py-2">
            @if (itemToDelete()) {
              Se eliminará la orden <strong>{{ itemToDelete()!.numeroReferencia }}</strong>.
            } @else {
              Se eliminarán <strong>{{ selectionCount() }}</strong> órdenes seleccionadas.
            }
...
export class OrdersListComponent extends GridBase<Orden> implements OnInit {
  ...
  readonly itemToDelete = signal<Orden | null>(null);
  ...
  protected override load(): void {
    ...
    this.error.set(null);
    this.http.get<Orden[]>(API).subscribe({
  ...
  goNew():              void { this.router.navigate(['/inventory/orders/new']); }
  goEdit(o: Orden):     void { this.router.navigate(['/inventory/orders', o.id, 'edit']); }
  goDetail(o: Orden):   void { this.router.navigate(['/inventory/orders', o.id]); }

  openDelete(o: Orden | null): void {
  ...
  confirmDelete(): void {
    ...
    if (o) {
      this.http.delete(`${API}/${o.id}`).subscribe({
        ...
        error: () => { this.deleteState.set('closed'); this.error.set('Error al eliminar la orden.'); },
      });
    } else {
      forkJoin([...this.selectedIds()].map(id => this.http.delete(`${API}/${id}`))).subscribe({
        next:  () => { this.deleteState.set('closed'); this.clearSelection(); this.load(); },
        error: () => { this.deleteState.set('closed'); this.error.set('Error al eliminar las órdenes.'); this.load(); },
      });
    }
  }
