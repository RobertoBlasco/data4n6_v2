      ...
      <div class="flex-1 overflow-auto min-h-0">
        ...
        @if (!loading() && !error() && totalRecords() === 0 && !searchQuery()) {
          <div class="flex flex-col items-center justify-center py-12 gap-3 text-muted-foreground">
            <ng-icon hlmIcon size="lg" name="lucidePackagePlus" class="opacity-25" />
                            ...
                            <tbody>
                              @for (l of linesCache().get(o.id)!; track l.id) {
                                <tr class="border-b border-border/50 last:border-0">
                                  ...
                                  <td class="py-1 pr-3 font-mono text-muted-foreground">{{ l.numeroSerie ?? '—' }}</td>
      ...
      <ng-template brnDialogContent>
        <hlm-dialog-content class="sm:max-w-md" [showCloseButton]="false">
          <div class="bg-destructive text-destructive-foreground flex items-center gap-2 px-4 h-11 -mx-6 -mt-6 mb-2 rounded-t-lg">
            <ng-icon hlmIcon size="sm" name="lucidePackagePlus" />
            <h2 class="text-sm font-semibold">¿Eliminar entrada de almacén?</h2>
          </div>
          <p class="text-sm text-muted-foreground py-2">
            @if (itemToDelete()) {
              Se eliminará la entrada <strong>{{ itemToDelete()!.numeroReferencia }}</strong>.
            } @else {
              Se eliminarán <strong>{{ selectionCount() }}</strong> entradas seleccionadas.
            }
...
export class WarehouseEntriesComponent extends GridBase<OrdenEntrada> implements OnInit {
  ...
  readonly itemToDelete  = signal<OrdenEntrada | null>(null);
  ...
  openDelete(o: OrdenEntrada | null): void {
  ...
  confirmDelete(): void {
    ...
    if (o) {
      this.http.delete(`${API}/${o.id}`).subscribe({
        ...
        error: () => { this.deleteState.set('closed'); this.error.set('Error al eliminar la entrada.'); },
      });
    } else {
      forkJoin([...this.selectedIds()].map(id => this.http.delete(`${API}/${id}`))).subscribe({
        next:  () => { this.deleteState.set('closed'); this.clearSelection(); this.load(); },
        error: () => { this.deleteState.set('closed'); this.error.set('Error al eliminar las entradas.'); this.load(); },
      });
    }
  }
