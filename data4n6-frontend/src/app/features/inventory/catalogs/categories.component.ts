...
import {
  lucideGrid3x3, lucidePlus, lucidePencil, lucideTrash2,
...
interface Category { id: string; code: string; name: string; description: string | null; }
      ...
      <div [class]="toolbarCls">
        @if (selectionCount() === 0) {
          <h1 class="text-sm font-semibold flex items-center gap-1.5">
            <ng-icon hlmIcon size="sm" name="lucideGrid3x3" />{{ labelPlural }}
          </h1>
          <div class="flex items-center gap-0.5">
            ...
            <button hlmBtn variant="ghost" size="icon" [class]="btnNewCls" title="Nueva categoría" (click)="openCreate()">
        ...
        @if (!loading() && !error() && totalRecords() === 0 && !searchQuery()) {
          <div class="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <button hlmBtn variant="outline" size="sm" (click)="openCreate()">
              <ng-icon hlmIcon name="lucidePlus" class="mr-1.5" />Añadir la primera
            </button>
          </div>
        }
        @if (!loading() && !error() && totalRecords() > 0) {
          <table hlmTable class="w-full">
            <thead hlmTHead [ngClass]="headerColor">
              <tr hlmTr>
                <th hlmTh class="w-28 cursor-pointer select-none" (click)="toggleSort('code', $event)">
      ...
      <ng-template brnDialogContent>
        <hlm-dialog-content class="sm:max-w-md" [showCloseButton]="false">
          <div class="bg-primary text-primary-foreground flex items-center gap-2 px-4 h-11 -mx-6 -mt-6 mb-4 rounded-t-lg">
            <ng-icon hlmIcon size="sm" name="lucideGrid3x3" />
            <h2 class="text-sm font-semibold">{{ editingItem() ? 'Editar categoría' : 'Nueva categoría' }}</h2>
          </div>

          <div class="flex flex-col gap-3">
            <div class="flex flex-col gap-1.5">
              <label hlmLabel>Código</label>
              <input hlmInput class="w-full" placeholder="Ej: EQUIPO_RED" [ngModel]="formCode()" (ngModelChange)="formCode.set($event)" />
            </div>
            <div class="flex flex-col gap-1.5">
              <label hlmLabel>Nombre <span class="text-destructive">*</span></label>
              <input hlmInput class="w-full" placeholder="Nombre de la categoría" [ngModel]="formName()" (ngModelChange)="formName.set($event)" />
            </div>
            <div class="flex flex-col gap-1.5">
              <label hlmLabel>Descripción</label>
              <input hlmInput class="w-full" placeholder="Descripción opcional" [ngModel]="formDescription()" (ngModelChange)="formDescription.set($event)" />
            </div>
            @if (formError()) {
          ...
          <div hlmDialogFooter class="mt-4">
            ...
            @if (!editingItem()) {
              <button hlmBtn variant="default" [disabled]="saving() || !formName()" (click)="saveAndNext()">
                Alta + Siguiente
              </button>
            }
            <button hlmBtn variant="default" [disabled]="saving() || !formName()" (click)="save()">
...
export class CategoriesComponent extends GridDialogBase<Category> implements OnInit {
  protected override readonly gridId        = 'inventory-categories';
  protected override readonly labelSingular = 'Categoría';
  protected override readonly labelPlural   = 'Categorías';
  protected override readonly icon          = 'lucideGrid3x3';
  ...
  protected override load(): void {
    ...
    this.error.set(null);
    this.api.get<Category[]>('/inventory/categorias-articulos').subscribe({
  ...
  override openEdit(item: Category): void {
    super.openEdit(item);
    this.formCode.set(item.code);
  ...
  private doSave(close: boolean): void {
    ...
    const req$ = existing
      ? this.api.put<Category>(`/inventory/categorias-articulos/${existing.id}`, body)
      : this.api.post<Category>('/inventory/categorias-articulos', body);
    ...
    req$.subscribe({
      next: saved => {
        this.allItems.update(list =>
          existing ? list.map(c => c.id === saved.id ? saved : c) : [...list, saved]
        );
  ...
  confirmDelete(): void {
    ...
    if (!item) return;
    this.api.delete(`/inventory/categorias-articulos/${item.id}`).subscribe({
      next:  () => { this.allItems.update(list => list.filter(c => c.id !== item.id)); this.deleteState.set(null); },
