...
import {
  lucideIdCard, lucideTrash2, lucidePlus,
  lucideRefreshCw, lucideDownload, lucideLayoutList,
  lucideExternalLink, lucideSearch, lucideX,
...
const API = 'http://localhost:8080/api/v1/catalog/docs';
type DialogState = 'open' | 'closed' | null;

interface DocType { id: string; description: string | null; active: boolean; }
...
@Component({
  ...
  providers: [provideIcons({
    lucideIdCard, lucideTrash2, lucidePlus, lucideExternalLink,
    lucideRefreshCw, lucideDownload, lucideLayoutList,
      ...
      <div class="flex-1 overflow-auto min-h-0">
        ...
        @if (!loading() && !error() && totalRecords() === 0) {
          <div class="flex flex-col items-center justify-center py-12 gap-3 text-muted-foreground">
            <ng-icon hlmIcon size="lg" name="lucideIdCard" class="opacity-25" />
            ...
            <tbody hlmTBody>
              @for (item of pageItems(); track item.id; let odd = $odd) {
                <tr hlmTr
                  ...
                  <td hlmTd class="text-xs text-primary">{{ item.description ?? '—' }}</td>
      ...
      <ng-template brnDialogContent>
        <hlm-dialog-content class="sm:max-w-sm" [showCloseButton]="false">
          <div class="bg-[#005a3b] text-white flex items-center gap-2 px-4 h-11 -mx-6 -mt-6 mb-4 rounded-t-lg">
            <ng-icon hlmIcon size="sm" name="lucideIdCard" />
            <h2 class="text-sm font-semibold">{{ editItem() ? 'Editar tipo' : 'Nuevo tipo de documento' }}</h2>
          </div>
          <div class="space-y-3">
            <div class="space-y-1.5">
              <label hlmLabel>Descripción <span class="text-destructive">*</span></label>
              <input hlmInput class="w-full" [(ngModel)]="formDesc" placeholder="Ej: DNI, Pasaporte..." />
            </div>
    ...
    <hlm-dialog [state]="deleteState()" (stateChanged)="onDeleteState($event)">
      <ng-template brnDialogContent>
        <hlm-dialog-content class="sm:max-w-sm" [showCloseButton]="false">
          ...
          <p class="text-sm text-muted-foreground py-2">Se eliminará <strong>{{ deleteItem()?.description }}</strong>. Esta acción no se puede deshacer.</p>
...
export class DocTypesListComponent extends GridBase<DocType> implements OnInit {
  protected override readonly gridId        = 'common-doc-types';
  protected override readonly labelSingular = 'Tipo Doc ID';
  protected override readonly labelPlural   = 'Tipos Doc ID';
  protected override readonly icon          = 'lucideIdCard';
  protected override readonly colMetaTableName = 't200_docs';
  ...
  readonly editItem   = signal<DocType | null>(null);
  readonly deleteItem = signal<DocType | null>(null);
  ...
  override ngOnInit(): void {
    this.loadGridPrefs();
    this.sortCriteria.set([{ field: 'description', dir: 'asc' }]);
    this.load();
  }
  ...
  protected override load(): void {
    ...
    this.error.set(null);
    this.http.get<DocType[]>(API).subscribe({
  ...
  openCreate(): void {
    ...
    this.formDesc = ''; this.formActive = true;
    this.formState.set('open');
  }

  openEdit(item: DocType): void {
    this.editItem.set(item);
    this.formDesc = item.description ?? ''; this.formActive = item.active;
    this.formState.set('open');
  }

  openDelete(item: DocType): void { this.deleteItem.set(item); this.deleteState.set('open'); }
  ...
  private doSave(onSuccess: () => void): void {
    if (!this.formDesc.trim()) return;
    this.saving.set(true);
    const body = { description: this.formDesc.trim(), active: this.formActive };
    const req = this.editItem()
      ? this.http.put<DocType>(`${API}/${this.editItem()!.id}`, body)
      : this.http.post<DocType>(API, body);
    req.subscribe({
