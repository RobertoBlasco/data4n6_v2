...
interface DocumentItem    { id: string; filename: string; description: string | null; createdAt: string; }
interface PendingDocument { tempId: string; file: File; filename: string; }

@Component({
  selector: 'app-t300-documents',
  standalone: true,
  ...
  template: `
    <div class="flex items-center gap-3 mb-4 pb-4 border-b border-border">
      ...
      <span class="text-muted-foreground" style="font-size:10px">PDF, Word, Excel, imágenes…</span>
    </div>
    <app-historical-grid [loading]="loading()" [empty]="items().length === 0 && pending().length === 0" emptyMessage="Sin documentos registrados">
      ...
      <tbody>
        ...
        @for (d of items(); track d.id; let odd = $odd) {
          <tr class="border-b border-border/40 last:border-0" [class.bg-surface-primary]="odd">
            <td class="px-2 py-1.5">
              <button hlmBtn variant="ghost" size="icon" class="size-6 text-destructive hover:text-destructive" (click)="delete(d.id)">
                <ng-icon hlmIcon size="sm" name="lucideTrash2" />
              </button>
            </td>
            <td class="px-3 py-1.5 text-[#005a3b] truncate max-w-[14rem]">{{ d.filename }}</td>
            <td class="px-3 py-1.5 text-muted-foreground whitespace-nowrap">{{ d.createdAt | date:'dd/MM/yy' }}</td>
          </tr>
        }
      </tbody>
    </app-historical-grid>
...
export class DocumentsComponent {
  ...
  private load(recordId: string, tableId: string): void {
    ...
    this.http.get<DocumentItem[]>(`${BASE}/inventory/documents?tableId=${tableId}&recordId=${recordId}`)
  ...
  private flushPending(recordId: string, tableId: string): void {
    this.pending().forEach(p => {
      ...
      this.http.post<DocumentItem>(`${BASE}/inventory/documents/upload`, fd)
        .subscribe({ next: d => this.items.update(l => [...l, d]) });
    });
    this.pending.set([]);
  }
  ...
  upload(event: Event): void {
    ...
    fd.append('file', file);
    this.http.post<DocumentItem>(`${BASE}/inventory/documents/upload`, fd)
      .subscribe({ next: d => this.items.update(l => [d, ...l]) });
  }

  delete(id: string): void {
    this.http.delete(`${BASE}/inventory/documents/${id}`)
      .subscribe({ next: () => this.items.update(l => l.filter(d => d.id !== id)) });
  }

  deletePending(tempId: string): void {
    this.pending.update(l => l.filter(d => d.tempId !== tempId));
  }
}
