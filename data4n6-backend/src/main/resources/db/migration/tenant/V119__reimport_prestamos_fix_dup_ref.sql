-- Fix V118: 78 of 98 source PRS headers were silently skipped because
-- tmp.t700_albaranes_agr contains duplicate referencia values and ON CONFLICT
-- DO NOTHING suppresses uniqueness violations on numero_referencia as well as PK.
-- Strategy: ROW_NUMBER() per referencia; rn>1 appends '-N' suffix to make it unique.

-- ── 1. Delete partial V118 import (reverse dependency order) ─────────────────
...
-- ── 2. Re-insert PRS order headers (suffix -N for duplicate referencias) ──────
-- ROW_NUMBER partitioned by referencia; rn=1 keeps the original, rn>1 gets '-N'.
-- Prefer row with a caso attached, then most recent fecha_inicio.
...
-- ── 3. Re-insert PRS header subtable — includes t100_casos_id ────────────────

INSERT INTO inventario.t600_ordenes_prestamo
       (t600_ordenes_id, adjudicatario_id, adjudicatario_tabla, fecha_devolucion, t100_casos_id)
SELECT lower(src.t700_albaranes_agr_id)::uuid,
...
-- ── 7. Re-insert DEV header subtable ─────────────────────────────────────────
...
-- ── 10. Re-insert event history for PRS lines ────────────────────────────────
...
-- ── 11. Re-insert event history for DEV lines ────────────────────────────────
...
-- ── 12. Recalculate estado_resultante for affected articles ──────────────────
...
-- ── 13. Recalculate descripcion_estado for affected articles ─────────────────
